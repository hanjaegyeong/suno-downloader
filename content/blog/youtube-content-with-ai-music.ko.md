...
![YouTube 공식 도움말 '생성형 AI 콘텐츠 사용 공개' 페이지 — AI 공개 요건, 변경·합성 콘텐츠 공시 대상, 모바일 업로드 시 공개 방법까지 안내](/blog/images/youtube-content-with-ai-music/01-yt-synthetic-disclosure.png)
...
내가 직접 채널에 곡을 올려보면 글에 적은 두 축 — Suno 라이선스 단계와 영상 안에서의 사용 형태 — 이 그대로 작동하는 게 보인다. 같은 곡이라도 Free 권리로 수익화 채널에 올릴 때와 Pro 권리로 출처·라이선스를 명시해 올릴 때, 업로드 단계에서 마음에 걸리는 정도가 확연히 달랐다. 그래서 나는 채널이 수익을 내기 시작하는 시점을 기준으로 Pro 권리 + 메타데이터 표기를 기본값으로 두는 쪽으로 운영을 잡았다.
