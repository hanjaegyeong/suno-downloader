...
내가 직접 Suno WAV를 받아 작업하다 보면 글에서 말한 그대로다. 손대지 않은 원본을 플레이리스트에 끼워 들으면 앞뒤 곡보다 한 단계 작고 답답하게 들리는 게 매번 반복됐다. LUFS 미터를 띄워보면 정말 -14보다 낮은 값에 머물러 있어서, 결국 후처리 없이 그냥 올리기는 어렵다는 결론에 매번 다시 도달하게 된다.
...
<!-- TODO SCREENSHOT: Ableton Live 12 타임라인에 Suno에서 받은 WAV가 올라간 화면 — 파형과 트랙 헤더 -->
...
<figcaption style="margin-top:12px;color:#6b6b80;font-size:13px;text-align:center;">Figure: 4가지 EQ 보정 포인트 — 30 Hz HPF, 300 Hz boxy cut, 7.5 kHz harsh tame, 12 kHz air shelf</figcaption>
</figure>

<!-- TODO SCREENSHOT: Logic Pro Channel EQ 화면 — high-pass 30 Hz, 300 Hz -2 dB, 12 kHz Air shelf +1 dB 커브 -->
## 컴프레서 — 보컬 다이내믹 조절
...
<!-- TODO SCREENSHOT: Ableton Live 12 Glue Compressor 세팅 — Threshold -18 dB, Ratio 2:1, Attack 10 ms -->
## 리버브와 딜레이 — 공간감 추가
...
<!-- TODO SCREENSHOT: Youlean Loudness Meter 2 화면 — Integrated -14.0 LUFS, True Peak -1.0 dBTP 도달 -->
...
<!-- TODO SCREENSHOT: REAPER 7의 마스터 트랙 FX 체인 — ReaEQ → ReaComp → ReaLimit → Loudness Meter 순서 -->
## 빠른 후처리 체크리스트
...
DAW 후처리는 본인이 직접 프롬프트로 만든 곡에 한해 정당한 워크플로우다. 내가 가사·디스크립터를 입력해 합성한 곡이라면 EQ·컴프·리미터를 거쳐 음원 플랫폼 업로드 사양까지 다듬는 작업은 명백한 권리다. Suno Pro 구독자가 무손실 WAV를 받아 본인 DAW에서 후처리하는 것이 이 글의 출발점이고, 같은 권리가 곡 백업·본인 포트폴리오 활용까지 자연스럽게 이어진다. 반대로 타인이 만든 곡을 받아 마스터링만 다시 걸어 재배포하거나 상업적으로 활용하는 행위는 별도 합의가 필요하다. 이 글의 체크리스트는 본인 창작물의 완성도를 높이는 도구이지, 다른 사람의 결과물을 라이선스 없이 가공하기 위한 절차가 아니다.
