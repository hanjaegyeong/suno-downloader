...
내가 직접 디스크립터를 바꿔가며 써보면 이 차이가 그대로 드러난다. 합성 신조어를 적으면 같은 프롬프트로 생성할 때마다 톤이 크게 출렁여서 베스트 테이크를 고르기 어려운 반면, 표준 장르명 하나로 좁히면 결과가 한 방향으로 모이는 게 매번 반복된다. 그래서 나는 niche 합성어로 헛돌기보다 표준 장르에서 출발해 무드·악기로 좁히는 쪽을 먼저 시도하게 된다.

<!-- TODO SCREENSHOT: 같은 가사에 장르만 다르게 한 3개 버전 출력 — Style of Music 필드만 변경한 비교 -->
...
![Suno Custom Mode Styles 입력란 — `synthwave, nostalgic, dreamy, 110 bpm, female vocal, analog synths` 디스크립터 입력 예시](/blog/images/ai-music-genre-catalog/02-style-of-music-ui.png)
...
<!-- TODO SCREENSHOT: 시대 디스크립터 비교 — 80s synthwave vs modern synthwave 결과 톤 차이 -->
...
<!-- TODO SCREENSHOT: 장르 조합 실패 사례 — 4개 이상 나열했을 때 스타일이 무시된 결과 -->
## 빠른 참고 카탈로그
...
장르 디스크립터 조합은 결국 본인이 직접 프롬프트를 입력해 곡을 만드는 작업이다. `k-pop 80s nostalgic dreamy`라는 4개 키워드를 본인 손으로 적어 합성한 결과물이라면, 그 곡에 대한 개인 청취·DAW 후처리·본인 포트폴리오 활용은 정당하다. Suno Pro 구독자라면 본인이 만든 곡을 무손실 WAV로 백업해 두는 것 역시 명백한 권리다. 반면 타인이 같은 디스크립터로 만든 결과물을 가져와 재배포하거나 상업적으로 활용하는 행위는 별도 합의가 있어야 한다. 카탈로그가 알려주는 것은 톤을 좁히는 방법이지, 다른 사람 작품을 가져다 쓰는 면허가 아니다.
