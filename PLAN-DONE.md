...
## 페이지 성능 실측·개선 — 홈 CLS 0.431→0.012 + Fly suspend (2026-06-07 완료)

> 시작·완료 모두 2026-06-07. **측정 우선** 원칙으로 실측한 결과 플랜의 "외부 폰트" 가설이 반증됨 — SEO 대상 정적 페이지(가이드·블로그)는 이미 우수(92~94점), 진짜 병목은 홈 CLS와 콜드스타트뿐. 무료 범위에서 둘 다 해결·라이브 검증 완료.

### 핵심 성과
- 홈 **CLS 0.431 → 0.0123** (라이브 워밍 측정 일관) — Core Web Vitals 통과(<0.1). Perf 57→84~91, LCP 4.1s→2.9~3.9s.
- 콜드스타트: `fly.toml` suspend 전환 — 첫 방문 풀부팅(curl 실측 3.5s) → 메모리 스냅샷 재개(~1s), `min_machines_running=0` 유지로 무료.

### 체크리스트 (전부 완료)
- [x] 현황 측정 — 홈·가이드·블로그 LCP·CLS·INP 실측 (로컬 Lighthouse, 모바일). PSI 키 차단(429) 우회
- [x] 외부 폰트 로딩 점검 — 가이드·블로그는 `display=swap`으로 이미 우수, 폰트는 비병목 (가설 반증)
- [x] 이미지 최적화 점검 — 블로그 이미지 dimensions 누락이나 현 CLS 0이라 비병목(향후 보험)
- [x] render-blocking CSS/JS 점검 — 홈 unused JS 156KB는 AdSense, 압축은 이미 brotli
- [x] 비용 대비 효과 큰 항목부터 개선 적용 — 무료 2건 적용·라이브 재측정 검증 완료

### 적용한 개선
**① 홈 CLS 0.431 → 0.0123**
- 원인: `<div id="root">` SEO fallback의 헤더·히어로가 React 앱과 박스모델 불일치(특히 히어로 상단 패딩 56px 차) → `createRoot().render()`가 fallback을 통째 교체(hydrate 아님)하며 점프.
- 수정: `index.html` fallback 헤더·main·히어로를 App.jsx와 **동일 클래스 마크업**(`.wrap`/`.header-inner`/`.logo`/`.hero`)으로 재작성, 히어로 텍스트도 i18n `heroDesc`와 일치. 프로덕션은 `<head>`에 CSS `<link>`가 있어 fallback 페인트 시점에 클래스 적용됨. SEO 본문 전부 보존.
- 검증: 로컬 빌드+Lighthouse CLS `0.431→0`, 헤드리스 DOM 덤프로 React 실마운트 확인(거짓 양성 아님). 배포 후 라이브 워밍 2회 일관 `0.0123`.

**② 콜드스타트 — `fly.toml` `auto_stop_machines: 'stop'→'suspend'`**

### 실측 결과 (2026-06-07, 모바일 Lighthouse)
| 페이지 | Perf | LCP | CLS | 판정 |
|--------|------|-----|-----|------|
| 홈 (개선 전) | 57 | 4.1s | 0.431 | 나쁨 |
| 홈 (개선 후 라이브) | 84~91 | 2.9~3.9s | 0.0123 | 통과 |
| 가이드 (정적) | 94 | 2.4s | 0.005 | 우수(무수정) |
| 블로그 (정적) | 92 | 2.6s | 0 | 우수(무수정) |

### 진행 로그
| 시간 | 작업 내용 |
|------|----------|
| 2026-06-07 | `/start-phase` 착수. PSI 키 차단(429)→로컬 Lighthouse로 홈·가이드·블로그 실측 |
| 2026-06-07 | 측정이 가설 반증: 정적 페이지 우수, 홈 CLS·콜드스타트만 병목. 무료 운영 제약 확인 |
| 2026-06-07 | 홈 CLS 수정(fallback 상단 클래스 일치) + fly.toml suspend, 로컬 검증 후 commit·push |
| 2026-06-07 | 배포 후 라이브 재측정 CLS 0.0123 확인. AdSense 오판 정정(미등록) |
| 2026-06-07 | 기간 걸리는 순위·체류 검증 항목 제거(플랜 원칙). 5/5 완료 → `/complete-phase` |

### 메모
- **측정이 가설을 뒤집은 사례**: 플랜은 "외부 폰트가 가이드·블로그 LCP를 망친다"고 의심했으나, 실측에서 정적 페이지는 이미 92~94점·CLS≈0이었고 진짜 문제는 홈(React) CLS였음. 추측 최적화를 막은 게 핵심.
- **CLS 근본 원인은 SPA fallback-swap**: SEO 크롤러용 정적 fallback을 React가 hydrate 없이 통째 교체하는 구조. 상단 박스모델만 일치시켜도 뷰포트 시프트 제거(하단 시프트는 CLS 미집계).
- **자기정정**: 배포 직후 1회차 CLS 0.17을 보고 "AdSense 광고 탓"이라 단정했으나 AdSense 미등록 → 오판. 워밍 2·3회차에서 0.0123로 드러남. 단일 데이터포인트 결론의 위험.
- **무료 운영 제약**: `min_machines_running=1`(상시 과금) 대신 `suspend`로 콜드스타트만 완화. 완전 0 제거는 유료 영역이라 미적용.
...
### 핵심 성과
- 7편 × ko·en = 14개 글에 **1인칭 경험 문단** 삽입. 실경험 없는 LLM의 허구(가짜 수치·채널명·날짜·일화) 일절 배제 — 각 글의 검증된 사실을 1인칭 관점으로만 재서술해 Experience 신호를 정직하게 강화
- **실측 스크린샷 2장** 추가 (Chrome 헤드리스 캡처): Udio 공식 가격 페이지, YouTube 공식 정책 도움말("생성형 AI 콘텐츠 사용 공개") → 기존 공식 페이지 패턴과 일관
- sitemap 검증 (30 엔트리·블로그 14 URL 포함)

### 체크리스트 (완료/미완 실제 상태)
- [x] 블로그 7편에 1인칭 실제 경험 문단 삽입 (ko·en 14개, 글의 기존 사실 기반·허구 수치 없음, 빌드 렌더 확인)
- [x] 스크린샷 연결 — Udio 가격 (실제 udio.com/pricing 헤드리스 캡처, 04b-udio-pricing.png)
- [x] 스크린샷 연결 — YouTube 합성콘텐츠 공시 (공식 도움말 정책 페이지 캡처, 01-yt-synthetic-disclosure.png)
- [—] 스크린샷 연결 — DAW 화면 (사용자 결정: 캡처 안 함. DAW 미설치·위조 부적절 → 글의 커스텀 SVG EQ 다이어그램으로 대체 유지)
- [~] 영어판 7편 검토 (신규 1인칭 문단 영문 교정 완료 — fluent. 전체 편집 검토는 사용자 최종 판단으로 남김)
- [x] GSC sitemap 검증 (30 엔트리·블로그14 확인, 재제출은 GSC 로그인 필요 — 사용자 처리)

### 진행 로그
| 시간 | 작업 내용 |
|------|----------|
| 2026-06-06 | 플랜 시작 — PLAN.md에서 추출 |
| 2026-06-07 | Chrome 헤드리스로 Udio 가격·YouTube 공식 정책 페이지 실측 캡처 → 4개 md(ko·en) 연결, 빌드 검증 |
| 2026-06-07 | sitemap 검증(블로그 14 URL 포함). DAW는 미설치로 위조 대신 보류 처리 |
| 2026-06-07 | 3개 병렬 에이전트로 7편×2언어 1인칭 경험 문단 삽입(위조 금지 제약), 14개 전수 검증 + 빌드 렌더 확인 |
| 2026-06-07 | DAW 스크린샷 사용자 결정(캡처 안 함)으로 제외 처리 후 완료 |

### 잔여(백로그 이관)
- 영어판 전체 편집 검토 / GSC sitemap 실제 재제출 (콘솔 로그인 필요)
- 스크린샷·캡처 작업은 향후 플랜에서 제외(사용자 결정) — 원본성은 SVG·1인칭·공식 페이지로 충족
